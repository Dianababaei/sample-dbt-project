...
with portfolio_vs_benchmark as (
    select * from BAIN_ANALYTICS.DEV.int_portfolio_vs_benchmark
),
...
final as (
    select
        md5(cast(coalesce(cast(c.portfolio_id as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(c.valuation_date as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as performance_key,
        p.portfolio_name,
        p.portfolio_type,
        p.fund_id,
        c.*,
